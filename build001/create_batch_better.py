...
import numpy as np
df = pd.read_csv("data/OMXS30c.csv", sep=";", skiprows=1)
df = df.sort_values('Date')
df = df.replace(0,np.nan)
df = df.dropna(how='all',axis=0)
#######
#print(df.head())
#high_prices = df.loc[:,'High price'].values()
high_prices = df['High price'].values
...
####
data = pd.DataFrame({'p':mid_prices}) #create a df with time series
data = data.dropna(how='all',axis=0)
mid_prices = data.values

#print(df['new'])
#plt.plot(range(df.shape[0]),(high_prices+low_prices)/2.0)
#plt.show()

# creating Test & Train Data

middle = int(len(mid_prices)/1.1)
print(middle)
"""for i in range(len(mid_prices[:middle])):
    print(mid_prices[i])"""
train_data = mid_prices[:middle]
test_data = mid_prices[middle:]
...
"""scaler.fit(train_data[di+smoothing_window_size:,:])
    #print(train_data[di+smoothing_window_size:,:])
#scaler.fit(train_data[di+smoothing_window_size:,:])
...
num_nodes = [100,300,150] # Number of hidden nodes in each layer of the deep LSTM stack we're using
...
#### def parameter of LSTM and Regession layer
...
#### calc LSTM OUTPUT - Feeding into Regession layer
...
test_points_seq = np.arange(middle,int(len(all_mid_data)-1-n_predict_once),50).tolist()
print(len(test_points_seq))
...
print(len(mse_test_loss_seq))
best_epoch = list(mse_test_loss_seq).index(np.min(mse_test_loss_seq))


best_prediction_epoch = best_epoch#int(epochs*0.8) # replace this with the epoch that you got the best results when running the plotting code
...
plt.xlim(middle,(len(all_mid_data)))
...
plt.title('Best Test Predictions Over Time'+str(best_prediction_epoch),fontsize=18)
plt.xlabel('Date',fontsize=18)
plt.ylabel('Mid Price',fontsize=18)
plt.xlim(middle,(len(all_mid_data)))

plt.savefig("mydream.png", dpi=300)
plt.show()
...
plt.plot(range(len(test_data)), test_data, label="test_data")
#plt.plot(range(len(test_data)), test_data, label="test_data")
